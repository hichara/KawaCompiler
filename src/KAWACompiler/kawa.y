...
ProgramUnity
	:Package ImportDeclaration Modifiers ClassDeclaration ProgramUnity {
		$4->setModifier($3);
		$4->setImports(*$2);
		program->addPackageWithClass($1, $4);
		$$ = program;
	}
	| Package ImportDeclaration Modifiers InterfaceDeclaration ProgramUnity{
		$4->setModifier($3);
		$4->setImports(*$2);
		program->addPackageWithInterface($1, $4);
		$$ = program;
	}
	| {$$ = program;}
;
...
TablesIndexe
	: '[' ENTIER ']' TablesIndexe {
		$4->setIndexAddIntAtFirstPosition($2);
		$$ = $4;
	}
	| {
		KT_IndexedArray* var = new KT_IndexedArray;
		$$=var;
	}
;
...
Modifiers
	: Modifiers Modifier {
		...
		if($2->getVisibility() > 0){
			$1->setVisibility($2->getVisibility());
		}
	}
	| {
		KT_Modifier* modifier = new KT_Modifier;
		$$=modifier;
	}
;
...
Extends
	: TEXTENDS Type {
		vector<string*> typeName = $2->getTypeName();
		string* name = new string;
...
ClassDeclaration
	: TCLASS ID Extends Implements ClassBody {
		$$ = new KT_Class;
		$$->setName($2);
		$$->setParentClass($3);
...
InterfaceDeclaration
	: TINTERFACE ID ExtendsList InterfaceBody {
		$$ = new KT_Interface;
		$$->setName($2);
...
MemberDec
	: Modifiers Type ID VariableInitializer VariableDeclaratorList ';' {
		/*PARSER_MemeberDec*/
		$$ = new PARSER_MemberDec;
	}
	| Modifiers Type ID FormalParameters Block {
		KT_SimpleMethod* methode = new KT_SimpleMethod;
		methode->setIndexParser(2);
		methode->setName($3);
		methode->setModifier($1);
		methode->setParams(*$4);
		methode->setType($2);
		methode->setBlock($5);
		$$ = methode;
	} /* methode avec type de retour */
	| Modifiers TVOID ID FormalParameters Block {
		KT_Type* type = new KT_Type;
		vector<string*>* typeName = new vector<string*>;
    string* svoid = new string("void");
		typeName->push_back(svoid);
	...
	| Modifiers ID FormalParameters Block {
	  ...
	  constructor->setIndexParser(1);
		constructor->setName($2);
...
Prototypes
	...
	| {
		vector<KT_Prototype*>* var = new vector<KT_Prototype*>;
		$$ = var;
	}
;

Prototype
	: Modifiers Type ID FormalParameters';' {
		$$ = new KT_Prototype;
		$$->setName($3);
		$$->setReturnType($2);
		$$->setModifier($1);
...
VariableDeclaratorList
	: ',' VariableDeclarator VariableDeclaratorList {
		$3->AddVariableAtFirstPosition($2);
		$$ = $3;
	}
	| {
		$$ = new PARSER_Field;
...
FormalParameters
	: '(' FormalParameterDecls ')' {$$ = $2;}
	| VoidFormalParametrs {
		vector<KT_Param*>* vect = new vector<KT_Param*>;
		$$ = vect;
	}
;
...
FormalParametersCalledMethod
	: '(' FormalParametersCalledMethodDecls ')' {$$ = $2;}
	| VoidFormalParametrs {$$ = $1;}
;

FormalParametersCalledMethodDecls
	: ID '[' ENTIER ']' TablesIndexe FormalParameterCalledMethodDeclsRest {
		KT_ID* id = new KT_ID;
		vector<string*> name;
		name.push_back($1);
		id->setValue(name);
		$5->setIndexAddIntAtFirstPosition($3);
		KT_ParamsMethodCall* param = new KT_ParamsMethodCall;
		param->setIndexedArray($5);
		param->setExpression(id);
		vector<KT_ParamsMethodCall*>* vect= new vector<KT_ParamsMethodCall*>;
		vect->push_back(param);
		for (vector<KT_ParamsMethodCall*>::iterator it = $6->begin(); it != $6->end(); ++it){
			KT_ParamsMethodCall* n = (*it);
			vect->push_back(n);
		}
		$$ = vect;
	}
...
FormalParameterDecls
	: VariableModifiers Type VariableDeclaratorId FormalParameterDeclsRest {
		$2->setArrayDim($3->getParamType()->getArrayDim());
		$3->setParamType($2);
		$3->setParamModifier($1);
		vector<KT_Param*>* vect = new vector<KT_Param*>;
		vect->push_back($3);
		for (vector<KT_Param*>::iterator it = $4->begin(); it != $4->end(); ++it){
			KT_Param* n = (*it);
			vect->push_back(n);
		}
		$$ = vect;
...
FormalParameterCalledMethodDeclsRest
	: ',' FormalParametersCalledMethodDecls {$$ = $2;}
  | {
		vector<KT_ParamsMethodCall*>* vect = new vector<KT_ParamsMethodCall*>;
		$$ = vect;
...
Statements
	...
	| {
		vector<KT_Statement*>* var = new vector<KT_Statement*>;
		$$ = var;
	}
;
...
Statement
  ...
  | PrintF {$$=$1;}
  | PrintI {$$=$1;}
  | TFINAL ID '.' ID Ids Tables VariableDeclarator VariableDeclaratorList ';' {
		vector<string*> name;
		name.push_back($2);
		name.push_back($4);
  ...
  | ID '.' ID Ids Tables VariableDeclarator VariableDeclaratorList ';' {
		...
		type->setTypeName(name);
		PARSER_Field* pf = new PARSER_Field;
		pf->AddVariableAtFirstPosition($6);
		pf->addVectorVariable($7);
		pf->upDateType(type);
		vector<KT_Variable*> vars = pf->getVariables();
		vector<KT_Statement*> stats;
		for (vector<KT_Variable*>::iterator it = vars.begin(); it != vars.end(); ++it){
  ...
  | TFINAL ID Tables VariableDeclarator VariableDeclaratorList ';' {
		vector<string*> name;
		name.push_back($2);
		KT_Type* type = new KT_Type;
		type->setBasicType(false);
		...
		type->setTypeName(name);
		PARSER_Field* pf = new PARSER_Field;
		pf->AddVariableAtFirstPosition($4);
		pf->addVectorVariable($5);
		pf->upDateType(type);
		pf->setFinal(true);
		vector<KT_Variable*> vars = pf->getVariables();
		vector<KT_Statement*> stats;
  ...
  | ID Tables VariableDeclarator VariableDeclaratorList ';' {
		...
		type->setTypeName(name);
		PARSER_Field* pf = new PARSER_Field;
		pf->AddVariableAtFirstPosition($3);
		pf->addVectorVariable($4);
		pf->upDateType(type);
		vector<KT_Variable*> vars = pf->getVariables();
		vector<KT_Statement*> stats;
		for (vector<KT_Variable*>::iterator it = vars.begin(); it != vars.end(); ++it){
  ...
  | BasicType Tables VariableDeclarator VariableDeclaratorList ';' {
	...
	| TFINAL BasicType Tables VariableDeclarator VariableDeclaratorList ';' {
		vector<string*> name;
		name.push_back($2);
		KT_Type* type = new KT_Type;
		type->setBasicType(true);
  ...
  | ID TablesIndexe '=' Expression ';' {
    ...
    affectation->setRExpression($4);
		affectation->setLExpression(id);
    affectation->setIndexedArray($2);
		$$=affectation;
	}
  | TTHIS '.' ID TablesIndexe '=' Expression ';' {
		string* thisname = new string("this");
    vector<string*> fullname;
		fullname.push_back(thisname);
		fullname.push_back($3);
    KT_ID* id = new KT_ID;
  	id->setValue(fullname);
    KT_Affectation* affectation = new KT_Affectation;
    affectation->setRExpression($6);
		affectation->setLExpression(id);
		affectation->setIndexedArray($4);
		$$ = affectation;
	}
  | Expression ';' {$$ = $1;}
	| TSUPER FormalParametersCalledMethod ';' {
		KT_Super* s = new KT_Super;
		s->setParams(*$2);
	}
  | BlockStatement {$$ = $1;}
  | ';' {$$ = new KT_Statement;}
...
Args
	: factFinal ArgsRest {
		KT_FactFinal* factFinal = static_cast<KT_FactFinal*>($1);
		vector<KT_FactFinal*>* args = new vector<KT_FactFinal*>;
		args->push_back(factFinal);
	...
	| {
		vector<KT_FactFinal*>* var = new vector<KT_FactFinal*>;
		$$ = var;
	}
;
...
ArgsRest
	: '+' factFinal ArgsRest {
		KT_FactFinal* factFinal = static_cast<KT_FactFinal*>($2);
		vector<KT_FactFinal*>* args = new vector<KT_FactFinal*>;
		args->push_back(factFinal);
...
ArgsI
	: ENTIER {
		KT_Entier* entier = new KT_Entier;
		entier->setValue($1);
		$$ = entier;
	}
	| ID {
...
BlockStatement
	: Block {
		$$ = new KT_BlockStatement;
	}
  | TIF '(' Expression ')' BlockStatement %prec THEN {
		KT_IfStatement* ifstatement = new KT_IfStatement;
		ifstatement->setCondition($3);
		ifstatement->setBStatement($5);
		$$=ifstatement;
	}
  | TIF '(' Expression ')' BlockStatement TELSE BlockStatement {
		KT_ifElseStatement* ifelseStatement = new KT_ifElseStatement;
		ifelseStatement->setCondition($3);
		ifelseStatement->setIfBStatement($5);
		ifelseStatement->setElseBStatement($7);
		$$ = ifelseStatement;
	}
  | TSWITCH '(' Expression ')' '{' SwitchBlockStatements '}' {
		KT_SwitchStatement* switchStatement = new KT_SwitchStatement;
		switchStatement->setCondition($3);
		switchStatement->setSwitchBStatements(*$6);
		$$ = switchStatement;
	}
  | TWHILE '(' Expression ')' BlockStatement {
		KT_WhileStatement* whileStatement = new KT_WhileStatement;
		whileStatement->setCondition($3);
		whileStatement->setWBStatement($5);
		$$ = whileStatement;
	}
  | TDO BlockStatement TWHILE '(' Expression ')' ';' {
		KT_WhileStatement* whileStatement = new KT_WhileStatement;
		whileStatement->setCondition($5);
		whileStatement->setWBStatement($2);
		$$ = whileStatement;
	}
  | TFOR '(' ForControl ')' BlockStatement {
		KT_ForStatement* forStatement = new KT_ForStatement;
		forStatement->setForControl($3);
		forStatement->setBlockStatement($5);
		$$ = forStatement;
	}
  | TBREAK ID ';' {
		KT_BreakStatement* breakStatement = new KT_BreakStatement;
		breakStatement->setIdExist(true);
		breakStatement->setId($2);
		$$ = breakStatement;
	}
  | TBREAK ';' {
		KT_BreakStatement* breakStatement = new KT_BreakStatement;
		breakStatement->setIdExist(false);
		$$ = breakStatement;
	}
	| TCONTINUE ID ';' {
		KT_ContinueStatement* continueStatement = new KT_ContinueStatement;
		continueStatement->setIdExist(true);
		continueStatement->setId($2);
		$$ = continueStatement;
	}
  | TCONTINUE ';' {
		KT_ContinueStatement* continueStatement = new KT_ContinueStatement;
		continueStatement->setIdExist(false);
		$$ = continueStatement;
	}
  | TRETURN Expression ';' {
		KT_ReturnStatement* returnStatement = new KT_ReturnStatement;
		returnStatement->setIsVoidReturn(false);
		returnStatement->setReturnExpression($2);
		$$ = returnStatement;
	}
  | TRETURN ';' {
		KT_ReturnStatement* returnStatement = new KT_ReturnStatement;
		returnStatement->setIsVoidReturn(true);
		$$ = returnStatement;
	}
;
...
SwitchBlockStatements
	...
	| {
		vector<KT_SwitchBlockStatement*>* var = new vector<KT_SwitchBlockStatement*>;
		$$ = var;
	}
;

SwitchBlockStatement
	: TCASE Expression ':' Statements {
		$$ = new KT_SwitchBlockStatement;
		$$->setCaseExist(true);
		KT_Block* block = new KT_Block;
		block->setStatements(*$4);
		$$->setBlock(block);
		$$->setExpression($2);
	}
	|  TDEFAULT ':' Statements {
...
ForControl
  ...
  | ';' Expression ';' {
		$$ = new PARSER_ForControl;
		$$->setCondition($2);
		PARSER_Field* initFor = new PARSER_Field;
		$$->setInitFor(initFor);
	}
...
ForVarControl
	: Type VariableDeclaratorId VariableDeclaratorList {
		$$ = new PARSER_Field;
		KT_Variable* var = $2->toVariable();
		$$->AddVariableAtFirstPosition(var);
		$$->addVectorVariable($3);
		$$->upDateType($1);
	}
	| Type VariableDeclaratorId '=' Expression VariableDeclaratorList {
		KT_Variable* variable = $2->toVariable();
		variable->setValue($4);
		$$ = new PARSER_Field;
		$$->AddVariableAtFirstPosition(variable);
		$$->addVectorVariable($5);
		$$->upDateType($1);
	}
...
ForUpdate
	: Expression StatementExpressionList {
		...
		update->push_back($1);
		for (vector<KT_IDExpression*>::iterator it = $2->begin(); it != $2->end(); ++it){
			KT_IDExpression* n = (*it);
			update->push_back(n);
		}
		$$ = update;
	}
	| ID Ids TablesIndexe '=' Expression  StatementExpressionList {
		...
		id->setValue(name);
		KT_Affectation* affectation = new KT_Affectation;
		affectation->setRExpression($5);
		affectation->setLExpression(id);
		affectation->setIndexedArray($3);
		vector<KT_IDExpression*>* update = new vector<KT_IDExpression*>;
		update->push_back(affectation);
		for (vector<KT_IDExpression*>::iterator it = $6->begin();	it != $6->end(); ++it){
			KT_IDExpression* n = (*it);
			update->push_back(n);
		}
		$$ = update;
...
StatementExpressionList
	: ',' Expression StatementExpressionList {
		vector<KT_IDExpression*>* update = new vector<KT_IDExpression*>;
		update->push_back($2);
		for (vector<KT_IDExpression*>::iterator it = $3->begin(); it != $3->end(); ++it){
			KT_IDExpression* n = (*it);
			update->push_back(n);
		}
		$$ = update;
	...
	| ','  ID Ids TablesIndexe '=' Expression StatementExpressionList {
		...
		vector<KT_IDExpression*>* update = new vector<KT_IDExpression*>;
		update->push_back(affectation);
		for (vector<KT_IDExpression*>::iterator it = $7->begin(); it != $7->end(); ++it){
			KT_IDExpression* n = (*it);
			update->push_back(n);
		}
		$$ = update;
	...
	| {
		vector<KT_IDExpression*>* var = new vector<KT_IDExpression*>;
		$$ = var;
	}
;
...
FacteurEffect
	: FacteurEffect TORBINEQ ExpressionOr {
		KT_OrBinEq* expression = new KT_OrBinEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
  | FacteurEffect TXORBINEQ ExpressionOr {
		KT_XOrBinEq* expression = new KT_XOrBinEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
  | FacteurEffect TANDBINEQ ExpressionOr {
		KT_AndBinEq* expression = new KT_AndBinEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
  ...
  | FacteurEffect TMODEQ ExpressionOr {
		KT_ModuloEq* expression = new KT_ModuloEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
  | FacteurEffect TDIVEQ ExpressionOr {
		KT_DivisionEq* expression = new KT_DivisionEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
  | FacteurEffect TMULEQ ExpressionOr {
		KT_MultiplicationEq* expression = new KT_MultiplicationEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
  | FacteurEffect TMINUSEQ ExpressionOr {
		KT_SubstractionEq* expression = new KT_SubstractionEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
  | FacteurEffect TPLUSEQ ExpressionOr {
		KT_AdditionlEq* expression = new KT_AdditionlEq;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
ExpressionOr
	: ExpressionOr TOR ExpressionAnd {
		KT_ExpressionOr* expression = new KT_ExpressionOr;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
ExpressionAnd
	: ExpressionAnd TAND ExpressionOrLogic {
		KT_ExpressionAnd* expression = new KT_ExpressionAnd;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
ExpressionOrLogic
	: ExpressionOrLogic '|' ExpressionOrExLogic {
		KT_ExpressionOrLogic* expression = new KT_ExpressionOrLogic;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
ExpressionOrExLogic
	: ExpressionOrExLogic '^' ExpressionAndLogic {
		KT_ExpressionOrXLogic* expression = new KT_ExpressionOrXLogic;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
ExpressionAndLogic
	: ExpressionAndLogic '&' ExpressionEqNeq {
		KT_ExpressionAndLogic* expression = new KT_ExpressionAndLogic;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
ExpressionEqNeq
	: ExpressionEqNeq TCNE ExpressionCompEq {$$ = $3;}
	| ExpressionEqNeq TCEQ ExpressionCompEq {$$ = $3;}
	| ExpressionCompEq {$$ = $1;}
;

ExpressionCompEq
	: ExpressionCompEq TCGE TermePlus {
		KT_ExpressionGE* expression = new KT_ExpressionGE;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$=expression;
	}
	| ExpressionCompEq '>' TermePlus {
		KT_ExpressionGT* expression = new KT_ExpressionGT;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
	| ExpressionCompEq TCLE TermePlus {
		KT_ExpressionLE* expression = new KT_ExpressionLE;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
	| ExpressionCompEq '<' TermePlus {
		KT_ExpressionLT* expression = new KT_ExpressionLT;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
TermeDecal
	: TermeDecal TDECALNS TermePlus {$$ = $3;}
...
TermePlus
	: TermePlus '+' terme {
		KT_Addition* expression = new KT_Addition;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
	| TermePlus '-' terme {
		KT_Substraction* expression = new KT_Substraction;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
terme
	: terme '*' facteur {
		KT_Multiplication* expression = new KT_Multiplication;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
	| terme '/' facteur {
		KT_Division* expression = new KT_Division;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
	| terme '%' facteur {
		KT_Modulo* expression = new KT_Modulo;
		expression->setLExpression($1);
		expression->setRExpression($3);
		$$ = expression;
	}
...
facteur
	: '~' facteur %prec NBINAIRE {
		KT_NBinary* expression = new KT_NBinary;
		expression->setExpression($2);
		$$ = expression;
	}
	| '!' facteur %prec NLOGIC {
		KT_Negation* expression = new KT_Negation;
		expression->setExpression($2);
		$$ = expression;
	}
	| '-' facteur %prec MOINUS {
		KT_Minus* expression = new KT_Minus;
		expression->setExpression($2);
		$$ = expression;
	}
	| '+' facteur %prec PLUS {$$ = $2;}
	| ID TDEC {
		KT_PostDecrement* expression = new KT_PostDecrement;
		vector<string*> name;
		name.push_back($1);
		KT_ID* id = new KT_ID;
		id->setValue(name);
		expression->setExpression(id);
		$$ = expression;
	}
	| TDEC ID {
		KT_PreDecrement* expression = new KT_PreDecrement;
		vector<string*> name;
		name.push_back($2);
		KT_ID* id = new KT_ID;
		id->setValue(name);
	...
	| ID TINC {
		KT_PostIncrement* expression = new KT_PostIncrement;
		vector<string*> name;
		name.push_back($1);
		KT_ID* id = new KT_ID;
		id->setValue(name);
	...
	| TINC ID {
		KT_PreIncrement* expression = new KT_PreIncrement;
		vector<string*> name;
		name.push_back($2);
		KT_ID* id = new KT_ID;
		id->setValue(name);
...
factFinal
	: '(' Expression ')' {$$ = $2;}
	| ENTIER {
		KT_Entier* expression = new KT_Entier;
		expression->setValue($1);
		$$ = expression;
	}
	| REEL {
		KT_Reel* expression = new KT_Reel;
		expression->setValue($1);
		$$ = expression;
	}
	| STRING {
		KT_String* expression = new KT_String;
		expression->setValue($1);
		$$=expression;
	}
	| ConstructorCall {$$ = $1;}
	...
	| TTRUE {
		KT_Bool* expression = new KT_Bool;
	...
	| TNULL {
		KT_Null* expression = new KT_Null;
		$$ = expression;
	}
;

ConstructorCall
	: TNEW ID Ids FormalParametersCalledMethod {
		KT_ConstructorCall* expression = new KT_ConstructorCall;
		expression->addString($2);
		expression->addVectorString(*$3);
		expression->setParams(*$4);
		$$ = expression;
	}
;

MethodCall
	: ID FormalParametersCalledMethod {
		vector<string*> name;
		name.push_back($1);
		KT_MethodCall* expression = new KT_MethodCall;
		expression->setName(name);
		expression->setParams(*$2);
		$$ = expression;
	}
;

ArrayInitializer
	: TNEW BasicType '[' ENTIER ']'{
		KT_IndexedArray* indexedArray = new KT_IndexedArray;
		indexedArray->setDimension(1);
		vector<int> indexes;
		indexes.push_back($4);
		KT_Type* type = new KT_Type;
		type->setBasicType(true);
		vector<string*> typeName;
		typeName.push_back($2);
		type->setTypeName(typeName);
		type->setArrayDim(0);
		KT_ArrayInitilizer* expression = new KT_ArrayInitilizer;
		expression->setIndexedArray(indexedArray);
...
LinkedMethodVarCall
	: TTHIS '.' ID LinkedMethodVarCallList {
		KT_ID* var = new KT_ID;
		vector<string*> ids;
		ids.push_back($1);
		ids.push_back($3);
		var->setValue(ids);
		$4->push_back(var);
		KT_LinkedMethodOrVarCall* expression = new KT_LinkedMethodOrVarCall;
		expression->setMixedCall(*$4);
		$$ = expression;
	}
	| ID LinkedMethodVarCallList {
		...
		$2->push_back(var);
		KT_LinkedMethodOrVarCall* expression = new KT_LinkedMethodOrVarCall;
		expression->setMixedCall(*$2);
		$$ = expression;
	}
	| TTHIS '.' MethodCall LinkedMethodVarCallList {
		$3->setNameAddStringAtFirstPosition($1);
		$4->push_back($3);
		KT_LinkedMethodOrVarCall* expression = new KT_LinkedMethodOrVarCall;
		expression->setMixedCall(*$4);
		$$ = expression;
...
LinkedMethodVarCallList
	...
	| {
		vector<KT_MethodOrVarCall*>* vect = new vector<KT_MethodOrVarCall*>;
		$$ = vect;
	}
;
